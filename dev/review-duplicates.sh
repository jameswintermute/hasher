...
# Handles CSV reports generated by hasher, with colorized display, preview mode, and resumable checkpointing
...
awk -F, '/^"[0-9a-f]{64}"/ {
    ...
    gsub(/"/,"",$3); file=$3;
    gsub(/"/,"",$6); size=$6;
    if(file!="") print hash, file, size
}' "$REPORT_FILE" | while read -r hash file size; do
    FILE_MAP["$hash"]+="$file"$'\n'
    SIZE_MAP["$file"]="$size"
done
...
for hash in "${!FILE_MAP[@]}"; do
    filecount=$(echo -n "${FILE_MAP[$hash]}" | grep -c '^')
    if (( filecount > 1 )); then
        HASHES+=("$hash")
    fi
done
