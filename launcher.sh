...
show_quarantine_status() {
  ...
  if [ -n "$plan_file" ] && [ -s "$plan_file" ]; then
    ...
    # Try to estimate by summing file sizes via latest CSV if the plan is file paths.
    local latest_csv=""; latest_csv="$(ls -1t "$HASHES_DIR"/hasher-*.csv 2>/dev/null | head -n1 || true)"
    if [ -n "$latest_csv" ]; then
      plan_size_bytes="$(awk -v PF="$plan_file" -v CSV="$latest_csv" 'BEGIN{  FS=","; OFS="\t";  while ((getline < CSV) > 0) {    if (NR==1) {      for (i=1;i<=NF;i++) {        h=tolower($i); gsub(/^[ \t"]+|[ \t"]+$/,"",h);        if (h=="path") p=i;        if (h=="size_bytes") s=i;      }      if (!p || !s) { close(CSV); break }      continue    }    path=$p; gsub(/^"|"$/,"",path);    size=$s+0; sz[path]=size;  }  close(CSV);  total=0;  while ((getline line < PF) > 0) {    gsub(/\r$/,"", line);    if (line=="") continue;    if (line in sz) total+=sz[line];  }  print total}')"
    fi
