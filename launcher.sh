...
# - Start hasher with nohup (NAS-safe defaults) and verify PID
set -Eeuo pipefail
IFS=$'\n\t'; LC_ALL=C
...
# --- Start hasher (nohup) and verify PID ---
run_hasher_nohup() {
  local script="$1"
  local pid
  if [ -x "$script" ]; then
    info "Starting hasher (NAS-safe defaults): $script (nohup to $BACKGROUND_LOG)"
    nohup "$script" --nohup >>"$BACKGROUND_LOG" 2>&1 &
  else
    info "Starting hasher (NAS-safe defaults) via sh: $script (nohup to $BACKGROUND_LOG)"
    nohup sh "$script" --nohup >>"$BACKGROUND_LOG" 2>&1 &
  fi
  pid=$!
  sleep 0.4
  ...
  if [ -n "$csv" ] && [ -s "$csv" ]; then
    ...
    if [ "${lc:-0}" -gt 1 ]; then
      ...
      files="$(sample_files_quick)"
      info "Quick sample (depth≤2): at least ${files} files to scan (lower-bound)."
    fi
  else
    files="$(sample_files_quick)"
    if [ "${files:-0}" -gt 0 ]; then
      ...
      info "No previous CSV found; first full run time varies with disk & CPU."
    fi
  fi
...
run_hasher_interactive() {
  ...
  if [ -x "$script" ]; then
    ...
    info "Running hasher interactively via sh: $script"
    sh "$script"
  fi
}
...
action_delete_junk() {
  ...
  case "${mode,,}" in
    q|quarantine)
      qdir="$ROOT_DIR/quarantine-$(date +%F)"; ts="$(date +%F-%H%M%S)"; dest="$qdir/junk-$ts"
      mkdir -p "$dest"
