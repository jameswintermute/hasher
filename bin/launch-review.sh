#!/usr/bin/env bash
# launch-review.sh — Robust wrapper for menu Option 4
# Checks readiness, offers to run find-duplicates with a spinner, then launches review-duplicates.sh.
# BusyBox-friendly spinner. License: GPLv3
set -Eeuo pipefail
IFS=$'\n\t'; LC_ALL=C
...
spinner_start() {
  # prints spinner to stderr to avoid mixing with child stdout when needed
  { 
    i=0; frames='|/-\\'; while :; do 
      i=$(( (i + 1) % 4 )); printf "\r[%c] " "${frames:$i:1}" >&2; sleep 0.2; 
    done 
  } &
  echo $!
}
...
if [[ -z "${report:-}" || ! -s "$report" || ! report_has_groups "$report" ]]; then
  ...
  # 2) If we have a hasher CSV, offer to build the report now
  ...
  if [[ -n "${csv:-}" && -f "$csv" ]]; then
    ...
    if prompt_yn "Run 'find-duplicates.sh' now to generate the report?" "Y"; then
      ...
      info "Duplicates report generated."
      # refresh report pointer
      report="$(latest_duplicate_report || true)"
    else
