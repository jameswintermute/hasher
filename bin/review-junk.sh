...
# delete-junk.sh — remove OS/NAS junk safely (BusyBox-friendly)
# Usage:
#   delete-junk.sh [--paths-file FILE] [--include-recycle] [--verify-only|--dry-run|--force|--quarantine DIR]
# Defaults:
#   - If --paths-file is omitted, uses local/paths.txt (or ./paths.txt as fallback)
#   - Action defaults to --verify-only
#
# Quarantine:
#   - When --quarantine DIR is used, matched files/dirs are moved under DIR preserving relative paths.
...
# ── read roots robustly
ROOTS_TMP="$(mktemp)"
# strip CR, trim, drop blanks and comments
sed -e 's/\r$//' -e 's/^[[:space:]]\+//' -e 's/[[:space:]]\+$//' "$PATHS_FILE" \
  | awk 'NF>0 && $0 !~ /^[[:space:]]*#/' > "$ROOTS_TMP"
